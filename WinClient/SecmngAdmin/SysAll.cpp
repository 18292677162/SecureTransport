...
BEGIN_MESSAGE_MAP(CSysAll, CFormView)
	...
	ON_BN_CLICKED(IDC_BUTTON_CHECK, &CSysAll::OnBnClickedButtonCheck)
	ON_BN_CLICKED(IDC_BUTTON_REVOKE, &CSysAll::OnBnClickedButtonRevoke)
END_MESSAGE_MAP()
...
void CSysAll::OnBnClickedButtonCheck()
{
	// TODO:  密钥校验
	int ret = -1;
	ret = MngClient_Check(&pCltInfo);
	if (ret != 0) {
		AfxMessageBox("密钥校验失败！");
	}
	else {
		AfxMessageBox("密钥校验成功！");
	}
	return;
}


void CSysAll::OnBnClickedButtonRevoke()
{
	// TODO:  密钥注销
	int ret = -1;
	ret = MngClient_Revoke(&pCltInfo);
	if (ret != 0) {
		AfxMessageBox("密钥注销失败！");
	}
	else {
		AfxMessageBox("密钥注销成功！");
	}
	return;
}
